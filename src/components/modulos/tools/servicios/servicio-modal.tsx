...
import { useCreateServicio, useUpdateServicio } from '@/src/hooks/servicios/use-servicios'
...
export function ServicioModal({ item, onClose }: { item?: Servicio; onClose: () => void }) {
  const crear  = useCreateServicio()
  const editar = useUpdateServicio()
  const isEdit = !!item
      ...
      >
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          <div>
            ...
            <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder="Ej. Mantenimiento de lineas" style={INP} />
          </div>
          <div>
        ...
        <div className="px-5 py-4 flex gap-3 justify-end shrink-0"
          ...
          <button onClick={submit} disabled={isPending || !nombre.trim()}
            ...
            style={{ background: 'var(--color-primary)', color: '#fff', opacity: isPending || !nombre.trim() ? 0.6 : 1 }}>
