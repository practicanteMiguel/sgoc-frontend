...
import { useDeleteHerramienta } from '@/src/hooks/herramientas/use-herramientas'
import type { Herramienta } from '@/src/types/herramientas.types'

export function EliminarHerramientaModal({ herramienta, onClose }: {
  herramienta: Herramienta; onClose: () => void
}) {
    ...
    <ModalPortal onClose={onClose}>
      ...
      >
        <div className="px-6 pt-6 pb-4 flex flex-col items-center text-center">
          ...
          <div className="flex items-center gap-3 mt-4 p-3 rounded-lg w-full"
            ...
            <div className="text-left min-w-0">
              <p className="text-sm font-semibold truncate" style={{ color: 'var(--color-text-900)' }}>{herramienta.descripcion}</p>
              <p className="text-xs truncate" style={{ color: 'var(--color-text-400)' }}>{herramienta.codigo}</p>
            </div>
          </div>
        ...
        <div className="flex gap-3 px-6 py-4" style={{ borderTop: '1px solid var(--color-border)' }}>
          ...
          <button
            onClick={() => eliminar.mutate(herramienta.id, { onSuccess: onClose })}
            disabled={eliminar.isPending}
            className="flex-1 py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-opacity"
            style={{ background: 'var(--color-danger)', color: '#fff', opacity: eliminar.isPending ? 0.75 : 1 }}
          >
