...
// Campos a ocultar en los valores (datos sensibles)
...
export function AuditLogItem({ log }: AuditLogItemProps) {
  ...
  const moduleLabel   = moduleCfg?.label ?? log.module;
  ...
  const hasChanges = log.new_values || log.old_values;
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.borderColor = actionCfg.color;
        (e.currentTarget as HTMLElement).style.boxShadow  = `0 2px 12px ${actionCfg.border}`;
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {hasChanges && (
          <div className="mt-2">
            {/* UPDATE: mostrar qué cambió comparando old vs new */}
                ...
                {Object.keys(log.new_values)
                  ...
                  .map((k) => (
                    <div key={k} className="flex items-center gap-2 flex-wrap">
                      ...
                      <span
                        className="text-xs font-medium px-1.5 py-0.5 rounded line-through"
                        style={{
